...
function usage() {
    echo "ERROR: make: Usage: $(basename ${0}) <mod_dir>" >&2
    exit 1
}
...
function main () {
    ...
    cd "$(dirname ${0})"
    echo "INFO: make: script dir: $PWD" >&2
    echo "INFO: make: mod_dir: $mod_dir" >&2

    local packages_dir="./packages" 

    jq_path=$(which jq)
    if [[ "$jq_path" == "" ]]; then 
        echo ERROR: make: 'jq' command not found in PATH, try installing it with a package manager or setting its location in PATH
        exit 1
    fi
    ...
    if [[ ! -f "$modinfo_path" ]]; then 
        echo "ERROR: make: Could not locate modinfo.json at $modinfo_path" >&2
        exit 1
    fi
    ...
    if [[ "$old_packages" != "" && "$ONE_package_PER_VERSION" == true ]]; then
        echo "WARN: make: Removing old packages for version: $old_packages" >&2
    ...
    echo "INFO: make: finished making: $(readlink -f $package_path)" >&2
